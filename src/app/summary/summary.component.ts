...
export class SummaryComponent implements OnInit {
  ...
  totalCost: number = 0;
  totalCostObj = { min: 0, max: 0 };
  teamList = ['Web developer', 'Project manager', 'QA', 'UI/UX Designer'];
  completionDate = '';
  ...
  ngOnInit() {
    const selectedDesign = +this.mainService.designSelected;
    this.totalHours = this.mainService.devHours + this.mainService.nonDevHours;
    this.totalHoursObj.min = this.mainService.totalHoursObj.min + this.mainService.nonDevHoursObj.min;
    this.totalHoursObj.max = this.mainService.totalHoursObj.max + this.mainService.nonDevHoursObj.max;
    const { min, max } = this.mainService.designHoursObj;
    if (min > 0 && max > 0) {
      this.totalHours += ((min+max)/2) * selectedDesign;
      this.totalHoursObj.min += (min * selectedDesign);
      this.totalHoursObj.max += (max * selectedDesign);
    }
    const { appType, appName, costForSelectedApp, platformSelected } = this.mainService;
    const cost = costForSelectedApp[appType][appName.toLowerCase()];
    if (this.totalHours > 0) {
      this.totalCostObj = { min: Math.round(this.totalHoursObj.min * cost.min), max: Math.round(this.totalHoursObj.max * cost.max) };
      this.totalCost = Math.round((this.totalCostObj.min + this.totalCostObj.max) / 2);
    }
    if (platformSelected === 'Android') {
      this.teamList = ['Android developer', ...this.teamList];
    } else if (platformSelected === 'iOS') {
      this.teamList = ['iOS developer', ...this.teamList];
    } else if (platformSelected === 'Both') {
      this.teamList = ['Android developer', 'iOS developer', ...this.teamList];
    }
  ...
  setCompletionDate() {
    ...
    this.completionDate = `${month} ${day}, ${year}`;
    if (this.totalHours) {
      const { e } = this.mainService.getTotalHoursDetail();
      const totalDays = Math.round(Math.max(e.android.max, e.ios.max, e.design.max, e.web.max) / 7);
      if (totalDays > 0) {
  ...
  sendEmail(email: string) {
    if (email.includes('@') && email.split('@')[1].includes('.')) {
      this.loading = true;
      this.mainService.sendEmailWithEstimate(email)
      .subscribe(
        res => {
          this.loading = false;
          this.message = 'Congrats 🎉 pdf is sent to the email';
          this.generatePDF();
          this.user_email.nativeElement.value = '';
        },
        err => {
          this.loading = false; 
          console.log(err)
  ...
  generatePDF() {
    const { e, o } = this.mainService.getTotalHoursDetail();
    ...
    const designRow = [
      'Design', 
      e.design.min > 0 ? +(((e.design.min + e.design.max)/2).toFixed(1))*(+this.mainService.designSelected) : 0, 
      +((e.design.min).toFixed(1))*(+this.mainService.designSelected), 
      +((e.design.max).toFixed(1))*(+this.mainService.designSelected)
    ];
    ...
    if (body.length > 0) {
      ...
      doc.autoTable({
        head: [[{ content: `Rough Estimate ${this.mainService.appName} Users app Summary`, colSpan: 4, styles: { valign: 'middle', fontSize: 22, fillColor: 'ffffff', textColor: 'black' } }]],
      ...
      for (const key of ['android', 'ios', 'web', 'design']) {
        if (e[key].min > 0 && e[key].max > 0) {
          if (key == 'design' && this.mainService.designSelected == '0') {
            ...
            doc.autoTable({
              ...
              body: [
                ...
                ...this.getPdfDataForFeatures(key)
              ],
              showHead: 'firstPage',
              showFoot: 'lastPage',
              theme: 'plain',
  ...
  getPdfDataForFeatures(key: string): Array<any> {
    const devTimeArr = [];
    const allSelected = this.mainService.featuresSelected;
        ...
        if (key === 'design' && this.mainService.designSelected == '0') {
          ...
          for (const feature of selected.features) {
            if (!(feature[`${key}Min`] === 0 && feature[`${key}Max`] === 0) && feature.isSelected) {
              ...
              devTimeArr.push([
                feature.title, 
                key == 'design' ? +total*(+this.mainService.designSelected) : total, 
                key == 'design' ? +min*(+this.mainService.designSelected) : min, 
                key == 'design' ? +max*(+this.mainService.designSelected) : max
              ]);
            }
          }
        }
  ...
  calculateTotal (e, o): Array<string | object> {
    let total = e.web.min + e.web.max + e.android.min + e.android.max + e.ios.min + e.ios.max + ((e.design.min + e.design.max)*(+this.mainService.designSelected));
